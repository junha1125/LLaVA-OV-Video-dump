...
def _suppress_stderr():
    """Context manager that suppresses C-level stderr (e.g., FFmpeg warnings)."""
    import contextlib
...
def process_video_with_decord(video_file, data_args):
    ...
    # 1차 샘플링: video_fps=1이면 1초에 1프레임을 뽑겠다는 뜻이다. 30fps 비디오에서 30프레임마다 1장씩 추출하게 된다.
    ...
    # print(f"[VideoDebug] file={os.path.basename(video_file)}, "
    #       f"total={total_frame_num}, fps={vr.get_avg_fps():.1f}, "
    #       f"video_duration={video_time:.2f}s, "  
    #       f"step={avg_fps} | 1st sample {len(frame_idx)}f: {frame_idx}")

    if data_args.frames_upbound > 0:
        # 1차 샘플링 결과가 frames_upbound를 초과하거나, force_sample=True이면 → np.linspace로 균등 재샘플링
        if len(frame_idx) > data_args.frames_upbound or data_args.force_sample:
            uniform_sampled_frames = np.linspace(0, total_frame_num - 1, data_args.frames_upbound, dtype=int)
            frame_idx = uniform_sampled_frames.tolist()
            frame_time = [i/vr.get_avg_fps() for i in frame_idx]
            # print(f"[VideoDebug]   -> upbound={data_args.frames_upbound} resample {len(frame_idx)}f: {frame_idx}")
        else:
            pass
            # print(f"[VideoDebug]   -> upbound={data_args.frames_upbound} not exceeded, keep {len(frame_idx)}f")
