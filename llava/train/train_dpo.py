...
class DPODataCollator(DPODataCollatorWithPadding):
    ...
    def __call__(self, features: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if "image" in features[0]:
            ...
            # per-sample로 rejected가 있으면 그걸, 없으면 기본 image를 사용
            _imgrej_by_feat = []
            for f in features:
                if "image_rejected" in f:
                    _imgrej_by_feat.append(f["image_rejected"])  # [(tensor, size, modality)] 형태
                else:
                    _imgrej_by_feat.append(f["image"])    # 없으면 해당 샘플의 image로 대체

            # flatten하여 모델이 바로 consumption 가능하도록 정렬
