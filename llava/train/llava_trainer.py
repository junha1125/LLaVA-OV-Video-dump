...
class LLaVATrainer(Trainer):
    ...
    def _save_checkpoint(self, model, trial, metrics=None):
        if getattr(self.args, "tune_mm_mlp_adapter", False) or (
            ...
            if getattr(self.model.config, "add_visual_RoPE", False):
            ...
            cc_ids = getattr(self.model.config, "compressed_context_token_ids", [])
            if cc_ids:
                embed_w = self.model.get_input_embeddings().weight
                embed_w_full = maybe_zero_3(embed_w, ignore_status=True, name="compressed_context_embeddings").cpu()
