...
class DPOTrainer(Trainer):
    ...
    #     import warnings
    #     warnings.warn(
    #         "`tokenizer` is deprecated. Please use `processing_class` instead.",
    #         FutureWarning,
    #     )
    ...
    #     # 기존 tokenizer 인자를 processing_class 로 저장
    ...
    """
    wandb: Currently logged in as: junha1125 to https://api.wandb.ai. Use `wandb login --relogin` to force relogin
    wandb: Tracking run with wandb version 0.19.11
    wandb: Run data is saved locally in /mnt/ssd/junha/LLaVA-OV-ICLR26/wandb/run-20250525_131655-pb0lnyvk
    wandb: Run `wandb offline` to turn off syncing.
    wandb: Syncing run dpo-siglip2-base-Qwen2.5-0.5B
    wandb: ⭐️ View project at https://wandb.ai/junha1125/MllmRL-ICLR26
    wandb: 🚀 View run at https://wandb.ai/junha1125/MllmRL-ICLR26/runs/pb0lnyvk
    0%|                                                                                                                | 0/2807 [00:00<?, ?it/s]
    Token indices sequence length is longer than the specified maximum sequence length for this model (4102 > 2048). Running this sequence through the model will result in indexing errors
    Traceback (most recent call last):
    File "/mnt/ssd/junha/LLaVA-OV-ICLR26/llava/train/train_dpo.py", line 1865, in <module>
        train()
    File "/mnt/ssd/junha/LLaVA-OV-ICLR26/llava/train/train_dpo.py", line 1843, in train
        trainer.train()
    File "/mnt/ssd/junha/miniconda3/envs/llava/lib/python3.10/site-packages/transformers/trainer.py", line 2245, in train
        return inner_training_loop(
    File "/mnt/ssd/junha/miniconda3/envs/llava/lib/python3.10/site-packages/transformers/trainer.py", line 2514, in _inner_training_loop
        batch_samples, num_items_in_batch = self.get_batch_samples(epoch_iterator, num_batches, args.device)
    File "/mnt/ssd/junha/LLaVA-OV-ICLR26/trl/trainer/dpo_trainer.py", line 1175, in get_batch_samples
        policies, refs = self._generate_batch_samples(self.model, batch, device)
    File "/mnt/ssd/junha/LLaVA-OV-ICLR26/trl/trainer/dpo_trainer.py", line 1110, in _generate_batch_samples
        input_ids=batch["prompt_input_ids"],
    KeyError: 'prompt_input_ids'
    0860df050e32:449127:449282 [0] NCCL INFO [Service thread] Connection closed by localRank 1
    [2025-05-25 13:17:44,878] torch.distributed.elastic.multiprocessing.api: [WARNING] Sending process 449127 closing signal SIGTERM
    [2025-05-25 13:17:45,143] torch.distributed.elastic.multiprocessing.api: [ERROR] failed (exitcode: 1) local_rank: 1 (pid: 449128) of binary: /mnt/ssd/junha/miniconda3/envs/llava/bin/python
    Traceback (most recent call last):
    File "/mnt/ssd/junha/miniconda3/envs/llava/bin/torchrun", line 33, in <module>

    """

    # trl 0.12 do not have get_batch_samples, which means tansformers 4.51.1's trainer.py's get_batch_samples should be used.
    # ─────────────────────────────────────────────────────────────────────
    # 원본 생성 로직은 내부용으로 이름만 변경
    ...
    #     """Generate samples from the model and reference model for the given batch of inputs."""
    #     # peft + bf16 사용 시 hidden state 캐스팅을 위한 context manager
    #     generate_context_manager = nullcontext if not self._peft_has_been_casted_to_bf16 else torch.cuda.amp.autocast
    ...
    #         # reference 샘플 생성 (batch에 이미 있으면 재사용)
    #         if "reference_output" in batch:
    #             reference_output = batch["reference_output"]
    #         else:
    #             if self.ref_model is None:
    ...
    # # ─────────────────────────────────────────────────────────────────────
    # # Transformers 4.51.0 Trainer.get_batch_samples 시그니처 맞춤
    # def get_batch_samples(
    #     self,
    #     epoch_iterator,
    #     num_batches: int,
    #     device
    # ) -> Tuple[List[Tuple[List[str], List[str]]], None]:
    #     """
    #     1) epoch_iterator에서 num_batches 만큼의 raw batch(dict)를 꺼내고
    #     2) 각 batch에 대해 _generate_batch_samples를 호출해
    #        (policy_texts, ref_texts) 튜플을 리스트로 구한 뒤,
    #     3) list(zip(policy_lists, ref_lists)) 형태로 반환합니다.
    #     """
    #     # 1) raw_batches 수집
    #     raw_batches = []
    #     for _ in range(num_batches):
    #         try:
    #             raw_batches.append(next(epoch_iterator))
    #         except StopIteration:
    #             break

    #     # 2) generate & decode
    #     policy_lists: List[List[str]] = []
    #     ref_lists: List[List[str]] = []
    #     for batch in raw_batches:
    #         policies, refs = self._generate_batch_samples(self.model, batch, device)
    #         policy_lists.append(policies)
    #         ref_lists.append(refs)

    #     # 3) Trainer가 기대하는 반환 형태: (batch_samples, num_items_in_batch)
    #     #    DPO에서는 토큰 합계를 넘길 필요가 없으므로 None 처리
    #     batch_samples = list(zip(policy_lists, ref_lists))
    ...
    # transformers 4.51.1's Trainer.log 가 덮어져야하는데
    # trl 0.12의 DPOTrainer.log는 인푼 인자가 logs뿐임
    # 그래서 trl.13의 DPOTrainer.log 를 가져옴.. 이런거 보면 trnasformers 4.51.1 와 맞는 trl은 0.13 이상인것 같다
    # gpt는 0.16 이라고 말하긴 함
